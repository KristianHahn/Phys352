...
double rkO2_step(double x, double t, double f, double dt);

void rkO2_solver(double* x, double* t, double dt, int steps);

void rkO2_solver_fp(double* x, double* t, double (*dxdt)(double,double,double*), 
		     double* dxdt_parameters, double dt, int steps);

/* for two dependent variables */
double rkO2_step_2component(TwoComponent* x, double t, double f1, double f2, double dt);

void rkO2_solver_2component_fp(TwoComponent* x, double* t, 
...
void rkO2_step_4component(FourComponent* x, double* t, 
			   double fA, double fB, double fC, double fD,  
			   double dt); 

void rkO2_solver_4component_fp(FourComponent* x, double* t, 
